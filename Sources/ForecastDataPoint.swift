...
/// A ForecastDataPoint object contains various properties, each representing the average (unless otherwise specified) of a particular weather phenomenon occurring during a period of time: an instant in the case of `currently`, a minute for `minutely`, an hour for `hourly`, and a day for `daily`.
public struct ForecastDataPoint: Codable {
    ...
    /// The fractional part of the lunation number during the given day: a value of `0` corresponds to a new moon, `0.25` to a first quarter moon, `0.5` to a full moon, and `0.75` to a last quarter moon. (The ranges in between these represent waxing crescent, waxing gibbous, waning gibbous, and waning crescent moons, respectively.) Only on `daily`.
    let moonPhase: Double?

    /// The approximate direction of the nearest storm in degrees, with true north at 0° and progressing clockwise. (If `nearestStormDistance` is zero, then this value will not be defined.) Only on `currently`.
    let nearestStormBearing: Double?

    /// The approximate distance to the nearest storm in miles. (A storm distance of `0` doesn’t necessarily refer to a storm at the requested location, but rather a storm in the vicinity of that location.) Only on `currently`.
    let nearestStormDistante: Double?

    /// The columnar density of total atmospheric ozone at the given time in Dobson units.
    let ozone: Double?

    /// The amount of snowfall accumulation expected to occur, in inches. (If no snowfall is expected, this property will not be defined.) Only on `hourly` and `daily`.
